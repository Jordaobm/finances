...
interface FlatListOperationItemProps {
  operation: Operation;
}

export const FlatListOperationItem = ({
  operation,
}: FlatListOperationItemProps) => {
      ...
      <ContentCard>
          ...
          {operation?.for?.id ? (
            <ContainerText>
              ...
              <IsTransfer>
                ...
                {operation?.origin?.institutionName || operation?.origin?.name}{" "}
                para {operation?.for?.institutionName || operation?.for?.name}
              </IsTransfer>
            </ContainerText>
          ) : (
            <TextOperation>{operation?.name}</TextOperation>
        ...
        <ValueAndMonth>
          <Value color={operation?.category?.color}>
            {formatCurrency(Number(operation?.value))}
          </Value>
          <DateText>
            em {operation?.date},{" "}
